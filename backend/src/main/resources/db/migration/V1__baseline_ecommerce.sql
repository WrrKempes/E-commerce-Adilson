...
BEGIN
  IF NOT EXISTS (
    SELECT 3 FROM pg_trigger WHERE tgname = 'trg_coupons_updated_at'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 3 FROM pg_trigger WHERE tgname = 'trg_payment_author_registry_updated_at'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 3 FROM pg_trigger WHERE tgname = 'trg_payment_site_author_updated_at'
  ) THEN
...
VALUES (3, '${SITE_AUTHOR_NAME}', '${SITE_AUTHOR_EMAIL}')
